...
if (mod == 'VOL'):
    ...
    for vol_num in vollist:
        ...
        if (os.path.exists('INFO.OUT') == False):
            sys.exit('\n     ... Oops ERROR: There is NO "INFO.OUT" file in "'+ vol_num + \
            '" directory !?!?!?    \n')
    ...
    print\
    '\n Log(Final Residue in Ha):'+str(round(log10(sqrt(fopt)),3)),'\n'\
    '\n === Final Parameters ===='             \
    '\n E0 = '+str(round(E0,8))+' [Ha]'        \
    ...
    OUTOBJ.write(ET.tostring(root, method         ='xml',
                                   ...
                                   encoding       ='UTF-8'))
    OUTOBJ.close()
    print '\n     Optimized lattice parameters saved into "'+eos+'-optimized.xml" file.\n'
...
if (mod != 'VOL'):
    ...
    for i in range(1, NoP+1):
        ...
        if (os.path.exists('INFO.OUT') == False):
            sys.exit('\n     ... Oops ERROR: There is NO "INFO.OUT" file in "'+ dir_num + \
            '" directory !?!?!?    \n')
    ...
    OUTOBJ.write(ET.tostring(root, method         ='xml',
                                   ...
                                   encoding       ='UTF-8'))
    OUTOBJ.close()
    print '\n     Optimized lattice parameters saved into "'+mod.lower()+'-optimized.xml" file.\n'
#--------------------------------------------------------------------------------------------------
